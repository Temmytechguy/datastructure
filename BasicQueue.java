...
public class BasicQueue<X> {
	...
	public int size()
	{
		...
		//otherwise we add one to get the inclusive subtraction value rather than ecluding
		else {
			return end - front + 1;
		}
	}
	...
	public X deQueue()
	{
		...
		//otherwise grab the front if the queue, return it and adjust the front pointer
		else {
			item = data[front];
			data[front]= null;
			front++;
	...
	public X access(int position)
	{
		...
		{
			throw new IllegalArgumentException("No items in queue or position is greater than size of queue");
